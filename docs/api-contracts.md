...
| `C_CNK_EnableManagedMode` | `LIFECYCLE` | Borrows handles/callback pointers, then installs them as process-wide binding. Only the identical binding may be reaffirmed. | Rejects standalone initialized state, differing active binding, and incompatible cleanup-pending transitions. Failure does not replace allocator or handles. |
