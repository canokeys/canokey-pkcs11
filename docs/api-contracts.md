...
| `C_CNK_EnableManagedMode` | `LIFECYCLE` | Borrows handles/callback pointers, then installs a process-wide managed token binding. Windows may rotate card handles across same-card contexts; allocator callbacks must remain identical. | Rejects standalone initialized state, allocator changes, and incompatible cleanup-pending transitions. Success updates the active card handle for the caller's operation without replacing allocator ownership. |
